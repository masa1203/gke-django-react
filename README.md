...
ただ**まだ完全ではありません**が、少し経験がある方ならすぐに利用できるんじゃないかと思っています。

## 注意

これは未経験の趣味エンジニアがポートフォリオを作成しながら記録したものです。
不備があれば何卒御指摘をお願い致します。。
...
```sh
# connecsionNameの確認
$\gke-django-tutorial\gcloud sql instances describe websql
connectionName: gke-django-tutorial:asia-northeast1:websql

# インスタンスの接続
$\gke-django-tutorial\cloud_sql_proxy.exe -instances="gke-django-tutorial:asia-northeast1:websql"=tcp:5432
...
インスタンスレベルのアクセスについて secret を作成します。
...
```sh
$\gke-django-tutorial\kubectl create secret generic cloudsql --from-literal=username="master" --from-literal=password="websql-pass"
```
...
Django側で必要な環境変数を作成したSecretから参照していることが確認できます。
...
Serviceもyml形式で定義し、kubectlでGKEにリソースを追加することができます。
...
外部からのリクエストはServiceを呼び出し、Serviceの8000ポートからノードの8000ポート(backend-Djagno)に転送しています。
...
このあとbackendのService、frontendのService、それらを外部に公開するIngressを追加していきますが、Ingressを通して公開されるServiceはロードバランサからのヘルスチェックに応答する必要があります。
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: backend
        image: gcr.io/gke-django-tutorial/backend:latest
        imagePullPolicy: Always
...
現在のIngressの`ADRESS`はエファメラルIPアドレスなので、静的IPアドレスを予約してIngressが予約したIPアドレスを使用するようにIngressリソースを構成します。

```sh
# 静的IPアドレスの予約
\gke-django-tutorial\gcloud compute addresses create gke-django-tutorial-ip --global
Created [https://www.googleapis.com/compute/v1/projects/gke-django-tutorial/global/addresses/gke-django-tutorial-ip].
...
metadata:
  ...
  annotations:  # 追加
    kubernetes.io/ingress.class: gce
    kubernetes.io/ingress.global-static-ip-name: gke-django-tutorial-ip  # 追加
...
NAME              HOSTS   ADDRESS          PORTS   AGE
ingress-service   *       35.186.196.125   80      40m

# 確認
\gke-django-tutorial\curl http://35.186.196.125/api/
...
```
DNS名: (入力なし).sawakome.page
リソースレコードのタイプ: A
TTL: 5
IPv4アドレス: 35.186.196.125
```

CNAMEレコードの作成をします。

```
DNS名: www.sawakome.page
リソースレコードのタイプ: CNAME
TTL: 5
正規名: sawakome.page.
```
...
```sh
\gke-django-tutorial\curl http://sawakome.page/api/
...
metadata:
  ...
  annotations:
    ...
    networking.gke.io/managed-certificates: sawakome-certificate  # 使用したManagedCertificate
# ...[省略]
```
...
これで`https://sawakome.page/`にアクセスするとHTTPSができていることを確認できます。
...
ここまでコンテナラベルはなにも考えずに`latest`としてDeploymentを更新してきました。
今回は新しく
...
class App extends Component {
  ...
  getTodos() {
    axios
      .get("https://sawakome.page/api/")  // 変更
      .then(res => {
        this.setState({ todo: res.data });
      })
      .catch(err => {
...
CORS_ORIGIN_WHITELIST = (
    'http://localhost:3000','https://sawakome.page',
)
```
